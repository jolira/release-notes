(function (module) {
    ...
    var debug = require('./debug'),
        Batch = require("batch"),
        pattern = /https{0,1}:\/\/[^/]*\/projects\/(\w+)\/cards\/(\d+)/mg;
        ...
        jobs.forEach(function (job) {
            ...
            job.builds.forEach(function (build) {
                ...
                build.changeSet.items.forEach(function (item) {
                    ...
                    if (item.msg) {
                        var match = item.msg.match(pattern);

                        if (match) {
                            change.mingle = {
                                project:match[1],
                                card:match[2],
                                approved:isApproved(request, change.project, change.card)
                            };
                        }
                    }

                    change.date = new Date(item.date);
                    change.author = item.author.id;
